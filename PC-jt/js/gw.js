...
window.onload = function() {
	...
	var MNtimer = setInterval(oBtnNext.onclick, 3000);
	odiv.onmouseover = function() {
		clearInterval(MNtimer);
	};
	odiv.onmouseout = function() {
		MNtimer = setInterval(oBtnNext.onclick, 3000);
	};
	//		鞋子颜色
...
//		}	
//	}
	getbasicInfo();//商品基本信息方法，价格，编号，标签
	getSupInfo();//获取商品关联供应商信息
	getProductPermission();//获取商品浏览权限
	listGroupPic();//根据商品id获取商品不同颜色分组下的图片
	function getbasicInfo(){
		var pid="y0iuknebnr";
		_ProductBO_js.getProductBasicInfo(pid,function(data){
			
			$("#zhidaojia").text(data.p_price);
			$("#yangpinjia").text(data.p_price1);
			$("#chanpinbh").text(data.p_code);
			$("#shangpinming").html(data.p_name);
			$("#biaoqian").html(data.p_biaoqian);
		});
	}
	function getSupInfo(){
		//$(img).attr("src","路径");
		var pid="y0iuknebnr";
		_ProductBO_js.getSupInfo(pid,function(data){
			
			$("#gys_name").html(data.s_name);
			
			$("#gys_logo").attr("src",data.s_logo);
		});
	}
	function getProductPermission(){
		var pid="y0iuknebnr";
		_ProductBO_js.getProductPermission(pid,function(data){
			
			$("#cyquanxian").html(data);
		});
	}
	function listGroupPic(){
		var pid="y0iuknebnr";
		_ProductBO_js.listPicGroup(pid,function(data){
			alert(data);
			console.log(data);
		});
